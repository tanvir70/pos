        ...
        http
                ...
                .exceptionHandling(ex -> ex
                        .authenticationEntryPoint((request, response, authException) -> {
                            ...
                            response.getWriter().write("{\"status\":401,\"errorCode\":\"UNAUTHORIZED\",\"message\":\"Authentication required\"}");
                        })
                        .accessDeniedHandler((request, response, accessDeniedException) -> {
                            response.setContentType("application/json");
                            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                            response.getWriter().write("{\"status\":403,\"errorCode\":\"FORBIDDEN\",\"message\":\"Access denied: insufficient permissions\"}");
                        })
                )
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                        .requestMatchers("/api/auth/**").permitAll()
                        .requestMatchers("/api/barcode/**", "/api/lots/*/barcode-image").permitAll()
                        .requestMatchers("/api/backup/**").hasRole("OWNER")
                        ...
                        .requestMatchers("/api/dashboard/summary").hasRole("OWNER")
                        .requestMatchers("/api/inventory/quarantine/dispose").hasRole("OWNER")
